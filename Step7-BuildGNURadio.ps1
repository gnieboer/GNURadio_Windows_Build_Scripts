#
# Step7BuildGNURadio.ps1
#

# script setup
$ErrorActionPreference = "Stop"

# setup helper functions
if ($script:MyInvocation.MyCommand.Path -eq $null) {
    $mypath = "."
} else {
    $mypath =  Split-Path $script:MyInvocation.MyCommand.Path
}
if (Test-Path $mypath\Setup.ps1) {
	. $mypath\Setup.ps1 -Force
} else {
	. $root\scripts\Setup.ps1 -Force
}

# prep for cmake
if (!(Test-Path $root/src-stage3/build)) {
	cd $root/src-stage3
	mkdir build >> $null
} 

if (!(Test-Path $root/src-stage3/staged_install)) {
	cd $root/src-stage3
	mkdir staged_install >> $null
} 

# prep for cmake
SetLog "Build GNURadio Release"
if (!(Test-Path $root/src-stage3/staged_install/Release)) {
	cd $root/src-stage3/staged_install
	mkdir Release
} 
if (!(Test-Path $root/src-stage3/build/Release)) {
	cd $root/src-stage3/build
	mkdir Release
} 
cd $root/src-stage3/build/Release
if (Test-Path CMakeCache.txt) {Remove-Item -Force CMakeCache.txt} # Don't keep the old cache because if the user is fixing a config problem it may not re-check the fix

$env:PATH = "$root/bin;$root/src-stage1-dependencies/vcpkg/installed/x64-windows/bin;$root/src-stage1-dependencies/vcpkg/installed/x64-windows/lib;$pythonroot;$pythonroot/Dlls;$pythonroot\lib\site-packages\bin" + $oldPath
$env:PYTHONPATH=""
$env:PYTHONHOME="$pythonroot"
$env:PKG_CONFIG_PATH="$root/src-stage1-dependencies/vcpkg/installed/x64-windows/lib/pkgconfig"
$env:SPHINX_DIR="$pythonroot\lib\site-packages\bin"
# set PYTHONPATH=%~dp0..\gr-python$pyver\Lib\site-packages; %~dp0..\gr-python$pyver\dlls;%~dp0..\gr-python$pyver\libs;%~dp0..\gr-python$pyver\lib;%~dp0..\lib\site-packages;%~dp0..\gr-python$pyver\Lib\site-packages\pkgconfig;%~dp0..\gr-python$pyver\Lib\site-packages\gtk-2.0\glib;%~dp0..\gr-python$pyver\Lib\site-packages\gtk-2.0;%~dp0..\gr-python$pyver\Lib\site-packages\wx-3.0-msw;%~dp0..\gr-python$pyver\Lib\site-packages\sphinx;%~dp0..\gr-python$pyver\Lib\site-packages\lxml-3.4.4-py2.7-win.amd64.egg;%~dp0..\lib\site-packages\gnuradio\gr;%~dp0..\lib\site-packages\pmt;%~dp0..\lib\site-packages\gnuradio\blocks;%~dp0..\lib\site-packages\gnuradio\fec;%~dp0..\lib\site-packages\gnuradio\fft;%~dp0..\lib\site-packages\gnuradio\qtgui;%~dp0..\lib\site-packages\gnuradio\trellis;%~dp0..\lib\site-packages\gnuradio\vocoder;%~dp0..\lib\site-packages\gnuradio\audio;%~dp0..\lib\site-packages\gnuradio\channels;%~dp0..\lib\site-packages\gnuradio\ctrlport;%~dp0..\lib\site-packages\gnuradio\digital;%~dp0..\lib\site-packages\gnuradio\grc;%~dp0..\lib\site-packages\gnuradio\filter;%~dp0..\lib\site-packages\gnuradio\analog;%~dp0..\lib\site-packages\gnuradio\wxgui;%~dp0..\lib\site-packages\gnuradio\zeromq;%~dp0..\lib\site-packages\gnuradio\pager;%~dp0..\lib\site-packages\gnuradio\fcd;%~dp0..\lib\site-packages\gnuradio\video_sdl;%~dp0..\lib\site-packages\gnuradio\wavelet;%~dp0..\lib\site-packages\gnuradio\noaa;%~dp0..\lib\site-packages\gnuradio\dtv;%~dp0..\lib\site-packages\gnuradio\atsc;%~dp0..\lib\site-packages\gnuradio\pmt
# test notes... the following qa batch files must have the pythonpath prepended instead of appended:
# qa_tag_utils_test.bat requires this pythonpath: Z:/gr-build/src-stage3/src/gnuradio/gnuradio-runtime/python;Z:\gr-build\src-stage3\build\Release\gnuradio-runtime\swig\RelWithDebInfo;Z:\gr-build\src-stage3\build\Release\gnuradio-runtime\python;;Z:\gr-build\src-stage3\build\Release\gnuradio-runtime\swig
# qa_socket_pdu_test.bat
# qa_burst_shaper_test.bat
#
# There are a couple pulls requests pending to fix several test failures, at those a couple are left
# qa_agc will also fail for legit reasons still unknown, even after alignment gets fixed.  agc3 isn't converging as fast as it should
# qa_tcp_server_sink will fail because it in TCP source/sink is incompatible with windows
# qa_file_source_sink will fail because of the way the locking/opening of temp files varies in python between windows and linux


$env:_CL_ = ""
$env:_LINK_ = ""
$ErrorActionPreference = "Continue"
cmake ../../src/gnuradio `
	-G $cmakeGenerator -A x64 `
	-DCMAKE_TOOLCHAIN_FILE="$root/src-stage1-dependencies/vcpkg\scripts\buildsystems\vcpkg.cmake" `
	-DCMAKE_PREFIX_PATH="$root\src-stage1-dependencies\vcpkg\installed\x64-windows" `
	-DCMAKE_INSTALL_PREFIX="$root/src-stage3/staged_install/Release/" `
	-DCMAKE_CXX_FLAGS=" /EHsc /MD /W1 /DGSL_DLL /DBOOST_BIND_GLOBAL_PLACEHOLDERS " `
	-DCMAKE_C_FLAGS=" /EHsc /MD /W1 /DGSL_DLL " `
	-DCMAKE_SHARED_LINKER_FLAGS=" /DEBUG /opt:ref,icf " `
	-DCMAKE_BUILD_TYPE="RelWithDebInfo" `
	-DZEROMQ_LIBRARY_NAME="libzmq-mt-4_3_3" `
	-Wno-dev
$ErrorActionPreference = "Stop"
	
# NOW we build gnuradio finally
Write-Host -NoNewline "Build GNURadio Release..."
Write-Host -NoNewline "building..." 
msbuild .\gnuradio.sln /m /p:"configuration=RelWithDebInfo;platform=x64" *>> $Log 
Write-Host -NoNewline "staging install..."
msbuild INSTALL.vcxproj  /m  /p:"configuration=RelWithDebInfo;platform=x64;BuildProjectReferences=false" *>> $Log 

# Then combine it into a useable staged install with the dependencies it will need
Write-Host -NoNewline "moving add'l libraries..."
cp -Recurse -Force $root\src-stage1-dependencies\vcpkg\installed\x64-windows\bin\*.dll $root\src-stage3\staged_install\Release\bin\  *>> $Log 
cp -Recurse -Force $root\src-stage1-dependencies\vcpkg\installed\x64-windows\bin\gdbus.exe $root\src-stage3\staged_install\Release\bin\  *>> $Log 
cp -Recurse -Force $root\src-stage1-dependencies\vcpkg\installed\x64-windows\bin\gdbus-codegen $root\src-stage3\staged_install\Release\bin\  *>> $Log 
cp -Recurse -Force $root/src-stage1-dependencies\vcpkg\installed\x64-windows\bin\plugins $root\src-stage3\staged_install\Release\bin\  *>> $Log 
cp -Recurse -Force $root\src-stage1-dependencies\vcpkg\installed\x64-windows\lib\gobject-introspection $root\src-stage3\staged_install\Release\lib\  *>> $Log 
cp -Recurse -Force $root\src-stage1-dependencies\vcpkg\installed\x64-windows\lib\girepository-1.0 $root\src-stage3\staged_install\Release\lib\  *>> $Log 
cp -Recurse -Force $root\src-stage1-dependencies\vcpkg\installed\x64-windows\lib\gdk-pixbuf-2.0 $root\src-stage3\staged_install\Release\lib\  *>> $Log 
cp -Recurse -Force $root/src-stage1-dependencies\vcpkg\installed\x64-windows\tools\glib\gsettings.exe $root\src-stage3\staged_install\Release\bin\  *>> $Log 
cp -Recurse -Force $root/src-stage1-dependencies\vcpkg\installed\x64-windows\share\glib-2.0 $root\src-stage3\staged_install\Release\share\  *>> $Log 
cp -Recurse -Force $root/src-stage1-dependencies\vcpkg\installed\x64-windows\share\gir-1.0 $root\src-stage3\staged_install\Release\share\  *>> $Log 
"complete"

Write-Host -NoNewline "moving python..."
Copy-Item -Force -Recurse -Path $pythonroot $root/src-stage3/staged_install/Release/tools/python3/  *>> $Log
Write-Host -NoNewline "moving utilities..."
Copy-Item -Force -Path $root\src-stage3\src\run_gr.bat $root/src-stage3/staged_install/Release/bin  *>> $Log
Copy-Item -Force -Path $root\src-stage3\src\run_GRC.bat $root/src-stage3/staged_install/Release/bin  *>> $Log
Copy-Item -Force -Path $root\src-stage3\src\run_gqrx.bat $root/src-stage3/staged_install/Release/bin  *>> $Log
Copy-Item -Force -Path $root\src-stage3\src\gr_filter_design.bat $root/src-stage3/staged_install/Release/bin  *>> $Log
Copy-Item -Force -Recurse -Path $root\src-stage3\icons $root/src-stage3/staged_install/Release/share  *>> $Log

# ensure the GR build went well by checking for newmod package, and if found then build
Validate  $root/src-stage3/staged_install/Release/share/gnuradio/modtool/templates/gr-newmod/CMakeLists.txt
New-Item -Force -ItemType Directory $root/src-stage3/staged_install/Release/share/gnuradio/modtool/templates/gr-newmod/build 
cd $root/src-stage3/staged_install/Release/share/gnuradio/modtool/templates/gr-newmod/build

$ErrorActionPreference = "Continue"
cmake ../ `
	-G $cmakeGenerator -A x64 `
	-DPYTHON_EXECUTABLE="$pythonroot\$pythonexe" `
	-DQA_PYTHON_EXECUTABLE="$pythonroot\$pythonexe" `
	-DCMAKE_PREFIX_PATH="$root\build\Release" `
	-DCMAKE_INSTALL_PREFIX="$root/src-stage3/staged_install/Release/" `
	-Wno-dev
$ErrorActionPreference = "Stop"
msbuild .\gr-howto.sln /m /p:"configuration=RelWithDebInfo;platform=x64" *>> $Log
msbuild INSTALL.vcxproj  /m  /p:"configuration=RelWithDebInfo;platform=x64;BuildProjectReferences=false" *>> $Log
$env:_CL_ = ""
$env:_LINK_ = ""

"complete"

""
"COMPLETED STEP 7: Core GNURadio has been built from source"
""

